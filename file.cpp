...
#define CHECK_STATUS(stat, message)  do {\
    if( true != (stat) ) {                        \
        if ("" != (message)) \
...
bool file_op::File::Read(const char *filename, unsigned char **data, int &size)
{
    *data = nullptr;
    ...
    CHECK_STATUS(nullptr != fp, "Open file " + std::string(filename) + " failed.");

    fseek(fp, 0, SEEK_END);
    size = ftell(fp);

    ret = fseek(fp, offset, SEEK_SET);
    CHECK_STATUS(0 == ret, "blob seek failure");

    dataTemp = (unsigned char *) malloc(size);
    CHECK_STATUS(nullptr != dataTemp, "buffer malloc failure.\n");
